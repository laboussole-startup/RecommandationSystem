...
def get_recommendations(centre_interet: List[str] = Query(..., min_length=1),user_diplome: list[str]=Query(..., min_length=1), historique_recherche: List[str] = Query(None), page: int = 1, page_size: int = 10):
    recommended_courses = recommend_courses(centre_interet,user_diplome, historique_recherche, page=page, page_size=page_size)
